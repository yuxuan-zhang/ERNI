...
window = scipy.signal.general_gaussian(51, p=0.5, sig=20)
filtered = scipy.signal.fftconvolve(window, y_data_array)
filtered = (np.average(y_data_array) / np.average(filtered)) * filtered
filtered = np.roll(filtered, -25)

print(len(filtered) == len(y_data_array))
# exp_y_index = scipy.signal.find_peaks_cwt(filtered, widths=np.arange(1, 100), max_distances=np.arange(1,100)*2)
...
# plt.plot(x_data_array, filtered, 'r-', label=_name)
# plt.plot(x_data_array[exp_y_index], filtered[exp_y_index], 'go')
