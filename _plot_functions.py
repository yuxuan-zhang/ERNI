...
def get_tot_trans_for_single_ele(_input_ele_str, _input_thick_mm, energy_max, energy_min, energy_sub):
    ...
    for ele in elements:
        # isotopes_list = list(dict.keys(iso_ratio_dicts[ele]))
        iso_ratio_list = list(dict.values(iso_ratio_dicts[ele]))
        iso_ratio_array = np.array(iso_ratio_list)
        iso_mass_list = list(dict.values(iso_mass_dicts[ele]))
        iso_mass_array = np.array(iso_mass_list)
        ele_at_ratio = formula_dict[ele] / sum_ratios

        # A part for getting atoms_per_cm3, this part is irrelevant to fitting parameters, and will be exported for fitting
        avo_divi_mass_iso_ele_dict[ele] = avogadro_number / (sum(iso_ratio_array * iso_mass_array) * ele_at_ratio)
        # if stacked_foil_boo == 'Y':
            # Multiple foils stacked
            # sample_density_dict[ele] = density_gcm3_dict[ele] * 1
        atoms_per_cm3_dict[ele] = density_gcm3_dict[ele] * avo_divi_mass_iso_ele_dict[ele]

        # Get sigma related terms
        file_names = _functions.get_file_path(_database, ele)
        x_energy, sigma_iso_ele_isodict, sigma_iso_ele_l_isodict, sigma_iso_ele_sum, df_raw_dict[ele] \
            = get_xy_from_database(iso_ratio_dicts[ele],
                                   thick_cm_dict[ele],
                                   file_names,
                                   energy_min,
                                   energy_max,
        ...
        sigma_iso_ele_l_eleisodict[ele] = sigma_iso_ele_l_isodict
        # One level dict of elemental array of (L * sigma * iso_ratio * ele_ratio)
        sigma_iso_ele_sum_l_eledict[ele] = sigma_iso_ele_sum * thick_cm_dict[ele]

        # Two level dict of isotopic array of (sigma * iso_ratio * ele_ratio)
        sigma_iso_ele_eleisodict[ele] = sigma_iso_ele_isodict
        # One level dict of elemental array of (sigma * iso_ratio * ele_ratio)
        sigma_iso_ele_sum_eledict[ele] = sigma_iso_ele_sum
    ...
    #     sample_density = density_gcm3_dict[ele]
